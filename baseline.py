...
# risky_CE 相关数据
risky_dir = "parity"
uce_parity_file = f"{risky_dir}/uce_parity.csv"
possible_uce_parity_file = f"{risky_dir}/possible_uce_parity.csv"
f1_score_uce_parity_file = f"{risky_dir}/f1_score_uce_parity.csv"

uce_parity_df = pd.read_csv(uce_parity_file)
uce_parity = uce_parity_df["uce_parity"].values

possible_uce_parity_df = pd.read_csv(possible_uce_parity_file)
possible_uce_parity = possible_uce_parity_df["possible_uce_parity"].values

f1_score_uce_parity_df = pd.read_csv(f1_score_uce_parity_file)
f1_score_uce_parity = f1_score_uce_parity_df["top_1000_f1_score"].values
...
sn_info = pd.read_csv("df_info.csv")
sn = list(sn_info["SN"])
first_ce_time_list = list(sn_info["LogTime"])
Manufacturer_list = list(sn_info["Manufacturer"])
sn_first_ce_time = {sn: first_ce_time for sn, first_ce_time in zip(sn, first_ce_time_list)}
sn_Manufacturer = {sn: manu for sn, manu in zip(sn, Manufacturer_list)}
@dataclass
class Config(object):
    """
    配置类, 用于存储和管理程序的配置信息
...
class FeatureFactory(object):
    ...
    @staticmethod
    def _get_static_features(window_df: pd.DataFrame) -> Dict[str, int]:
    ...
    def _get_spatio_features(self, window_df: pd.DataFrame) -> Dict[str, int]:
        ...
        :param window_df: 时间窗口内的数据
        :return: 空间特征
        - fault_mode_rank: rank 故障, 即多个 rank 发生故障
        - fault_mode_device: device 故障, 即多个 device 发生故障
        - fault_mode_bank: bank 故障, 即多个 bank 发生故障

        - fault_mode_singel_cell: 单 cell 故障, 只有一组 (row id, column id)
        ...
        """
        # TODO 改为sn_mode  finish
        spatio_features = {
            "fault_mode_rank": 0,
            "fault_mode_device": 0,
            "fault_mode_bank": 0,
            "fault_mode_singel_cell": 0,
        ...
        spatio_features["fault_row_min_columnids"] = min(fault_row_columnids) if len(fault_row_columnids) > 0 else -1
        ...
        spatio_features["fault_column_avg_rowids"] = max(fault_column_rowids) if len(fault_column_rowids) > 0 else 0
        spatio_features["fault_column_avg_rowids"] = min(fault_column_rowids) if len(fault_column_rowids) > 0 else -1
    ...
    def _get_err_parity_features(window_df: pd.DataFrame) -> Dict[str, int]:
        ...
        """
        # TODO dq/burst 最小距离的最小值、平均值
        # TODO 添加risky CE 
        # TODO 相同地址parity变化次数(最大、平均、累加、频率的最大值) 
        # TODO 相同地址 最近两次parityLogTime间隔的最大值、最小值
        # TODO include_uce_parity_ce_count
        # TODO all_valid_err_log_count
        ...
        err_parity_features["error_adj_dq_count"] = window_df["adj_dq_count"].values.sum()
        err_parity_features["error_adj_burst_count"] = window_df["adj_burst_count"].values.sum()
        ...
        # 计算相同地址 最近两次 parity 的 LogTime 间隔的最大值、最小值
        # all_valid_err_log_count、 include_uce_parity_ce_count
        # 添加risky CE 
        ...
        uce_parity_count = 0
        possible_uce_parity_count = 0
        f1_score_uce_parity_count = 0
        for index, row in window_df.iterrows():
            position, parity = row["position_and_parity"].rsplit('_',1)
            parity_int = np.int64(parity)
            logtime = row["LogTime"]
            all_valid_err_log_count += 1 if parity_int & PARITY_VALID_MASK > 0 else 0
            include_uce_parity_ce_count += 1 if parity_int & PARITY_UCE_MASK > 0 else 0
            uce_parity_count += 1 if parity_int in uce_parity else 0
            possible_uce_parity_count += 1 if parity_int in possible_uce_parity else 0
            f1_score_uce_parity_count += 1 if parity_int in f1_score_uce_parity else 0
        ...
        err_parity_features["possible_uce_parity_count"] = possible_uce_parity_count
        err_parity_features["f1_score_uce_parity_count"] = f1_score_uce_parity_count
    ...
    @staticmethod
    def _get_bit_dq_burst_info(parity: np.int64) -> Tuple[int, int, int, int, int, int, int, int, int]:
        """
        获取特定 parity 的奇偶校验信息
        ...
        for start_idx, end_idx, end_time in zip(
            ...
            for time_window_size in self.config.TIME_RELATED_LIST:
                ...
                temporal_features = self._get_temporal_features(
                    self, window_df_copy, time_window_size
                )
                spatio_features = self._get_spatio_features(self, window_df_copy)
...
class NegativeDataGenerator(DataGenerator):
    def _select_day(self, data: pd.DataFrame) -> Union[pd.DataFrame, None]:
        ...
        for month in [1,2,3,4]:
            ...
            # 分类工作日（0-4=周一到周五）和周末（5-6=周六、周日）
            weekdays = [d for d in dates if d.weekday() < 5]
            weekends = [d for d in dates if d.weekday() >= 5]
            
            # 强制选择1工作日+1周末日
            selected = [
                np.random.choice(weekdays, 1)[0],
                np.random.choice(weekends, 1)[0]
            ]
            # 补充剩余天数
            others = [d for d in dates if d not in selected]
            selected.extend(np.random.choice(others, 2, replace=False))
    ...
    def _process_file(self, sn_file: str) -> Union[pd.DataFrame, None]:
        ...
        if not self.ticket_sn_map.get(sn_name):
            ...
            # end_time = self.train_end_date - 45 * ONE_DAY
            # start_time = self.train_end_date - 55 * ONE_DAY
            # data = data[(data["LogTime"] <= end_time) & (data["LogTime"] >= start_time)]
            data = data[data["all_ce_count_1h"] >= 2]
            data = data.sort_values(by=["all_ce_count_1h"])[-20:]
            if data.empty:
...
class MFPmodel(object):
    ...
    def __init__(self, config: Config):
        ...
        self.model_stage1 = LGBMClassifier(**self.model_params)
        self.model_stage2 = LGBMClassifier(**self.model_params)
    ...
    def train(self, pos_num: int = 9820, k: float = 18) -> NoReturn:
        # 第一阶段训练（保持原逻辑）
        ...
        # 筛选正样本（原始正样本中的高置信度样本）
        pos_pred_proba = self.model_stage1.predict_proba(self.train_pos.drop(columns=["label", "LogTime"]))[:, 1]
        pos_indices = np.argsort(-pos_pred_proba)[:pos_num]
        selected_pos = self.train_pos.iloc[pos_indices]
        
        # 筛选负样本（原始负样本中的困难样本）
        neg_pred_proba = self.model_stage1.predict_proba(self.train_neg.drop(columns=["label", "LogTime"]))[:, 1]
        ...
        # 获取第一阶段预测概率作为权重
        stage2_weights = self.model_stage1.predict_proba(X_stage2)[:, 1]  # 获取预测为1的概率
        ...
        self.model_stage2.fit(
            X_stage2, 
            y_stage2,
            sample_weight=stage2_weights  # 关键修改点
        )

    def filter_condition(self, group):
        sn_name = group['sn'].iloc[0]
        first_ce_time = sn_first_ce_time.get(sn_name)
        Manufacturer = sn_Manufacturer.get(sn_name)
        if Manufacturer == "A":
            # time_split = 1000000
            return group
        elif Manufacturer == "B":
            ...
            return group
        group["first_ce_time_distance"] = group["log_time"] - first_ce_time
        group = group[group['first_ce_time_distance'] <= time_split]
        group = group.drop(columns=["first_ce_time_distance"])
        return group
...
if __name__ == "__main__":
    ...
    config = Config(
        data_path=os.path.join("D:/stage1_feather", f"type_{sn_type}"),  # 原始数据集路径
    ...
    model.train()  # 训练模型
    result = model.predict(threshold = 0.7625) # 使用训练好的模型进行预测
